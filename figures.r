...
library(shape)
plot(odat$b.pre,odat$id,pch=16,xlim=c(-1,1),col='black',yaxt='n')
abline(v=0,lty=2)
ff<-function(d){
  #    Arrows(c(d$b.pre,d$id),c(d$b.post,d$id))
  shape::Arrows(x0=d$b.pre,y0=d$id,x1=d$b.post,y1=d$id,arr.adj=0,arr.length=.5,col='firebrick3')
}
zz<-dlply(odat,.(id),.fun=ff)
axis(2,at=odat$id,labels=odat$var,las=1)
points(odat$b.pre,odat$id,pch=16,col='black')



setwd(figsdir)
pdf('herring_indices_accel.pdf',height=10,width=8)
par(mar=c(4,8,1,1))
plot(0,0,xlim=c(-1,1),yaxt='n',ylab='',xab='Slope',pch='.',col='white',ylim=c(1,16))
rect(-2,-2,0,30,col='darkseagreen1',border=NA)
rect(0,-2,2,30,col='lightgoldenrod1',border=NA)

points(odat$b.pre,odat$id,pch=15,cex=2,yaxt='n',col=alpha('darkblue',ifelse(odat$pv<.05,1,.4)))
abline(v=0,lty=2)
ff<-function(d){
  shape::Arrows(x0=d$b.pre,y0=d$id,x1=d$b.post,y1=d$id,arr.adj=1,arr.length=.5,col=alpha('darkblue',ifelse(d$pv<.05,1,.4)))
}
zz<-dlply(odat,.(id),.fun=ff)
axis(2,at=odat$id,labels=odat$var,las=1)
box()
...
plot(0,0,xlim=c(-.18,.05),yaxt='n',ylab='',xab='Slope',pch='.',col='white',ylim=c(1,16))
rect(-2,-2,0,30,col='darkseagreen1',border=NA)
rect(0,-2,2,30,col='lightgoldenrod1',border=NA)
...
abline(v=0,lty=2)
ff<-function(d){
  shape::Arrows(x0=d$b.pre,y0=d$id,x1=d$b.post,y1=d$id,arr.adj=1,arr.length=.5,col=alpha(as.character(d$cl),ifelse(d$pv.post<.05,1,.4)))
}
zz<-dlply(odat2,.(id),.fun=ff)
axis(2,at=odat2$id,labels=odat2$var,las=1)
box()
dev.off()
...
nms<-subset(bptdat,select=c('var','lbl'))
...
p1<-ggplot(a, aes(x=year, fill=num,y=yrscale))+
  ...
  geom_bar(data=subset(a,yrscale<0),stat='identity',aes(width=.7,order=num),col=NA,size=.0001)+
  theme(legend.position=c(.8,.7),text = element_text(size=8),axis.text.x = element_text(angle=0, vjust=1), panel.grid.major=element_blank(), panel.grid.minor=element_blank(),panel.background = element_blank(), legend.key.size =  unit(0.2, "in"),legend.text=element_text(size=6))+
...
setwd(figsdir)
pdf('herring_indices_barplot.pdf',height=8,width=12)
p1
p2
dev.off()
...
setwd(figsdir)
pdf('timeline.pdf',height=5,width=12)
...
points(bptdat$bpt2,rep(.99,dim(a)[1]),pch=18,cex=3,col=alpha(as.character(bptdat$cls),1))
points(bptdat$bpt2,rep(1,dim(a)[1]),pch=16,cex=3,col=alpha(as.character(bptdat$cls),1))
#points(bptdat$bpt2,rep(1,dim(a)[1]),pch=1,cex=3,col=alpha('black',.3),lwd=.5)
text(bptdat$bpt2,rep(1.08,dim(bptdat)[1]),labels=bptdat$lbl,srt=45,adj=0,cex=.8,col=as.character(bptdat$cls))
...
points(bptdat$bpt2,rep(1,dim(a)[1]),pch=bptdat$pc,cex=1.25,col='white')

par(xpd=TRUE)
cx<-2.85
plot(0,0,xlim=c(1965,2018),ylim=c(.95,2),las=1,axes=FALSE,xlab='',ylab='')
axis(1,at=seq(1965,2015,5))
points(bptdat$bpt2,rep(.95,dim(a)[1]),pch=18,cex=2.5,col=alpha(as.character(bptdat$cls),1))
points(bptdat$bpt2,rep(.97,dim(a)[1]),pch=16,cex=cx,col=alpha(as.character(bptdat$cls),1))
f<-function(d){
  lines(c(d$bpt2,d$bpt2),c(.95,1.18),col=as.character(d$cls),lwd=2)
}
zz<-dlply(bptdat,.(id),.fun=f)
text(bptdat$bpt2,rep(1.2,dim(bptdat)[1]),labels=bptdat$lbl,srt=35,adj=0,cex=.8,col=as.character(bptdat$cls))
points(bptdat$bpt2,rep(.97,dim(a)[1]),pch=bptdat$pc,cex=1.25,col='white')
