...
data$id<-gsub(' ','',paste(data$cruise,'_',data$setno,'_',data$spec_stage,'_',data$netmesh,'_',data$sample,'_',data$tday))

#GETS COLLECTION OF ALL SAMPLES
sampdat<-unique(data[,!(names(data) %in% c('sciname','comname','aphiaid','rank','valid_name','kingdom','phylum','order','family','genus','tsn','species','spec','totno','spec_stage'))])
...
print(unique(d$spec))
d<-subset(d,select=c('id','spec','spec_stage','totno','sciname','comname','aphiaid','rank','valid_name','kingdom','phylum','order','family','genus','tsn','species'))
d2<-merge(d,sampdat,all.x=FALSE,all.y=TRUE)
d2$totno<-ifelse(is.na(d2$totno)==TRUE,0,d2$totno)
d2$spec_stage<-ifelse(is.na(d2$spec_stage)==TRUE,unique(as.character(d$spec_stage)),as.character(d2$spec_stage))
...
crds<-SpatialPoints(data.frame(lon=data2$lon,lat=data2$lat),proj4string=CRS("+proj=longlat +datum=WGS84 +no_defs +ellps=WGS84 +towgs84=0,0,0"))
gint<-over(crds,cst)
data2$dum<-gint[,1]
data2<-subset(data2,is.na(dum)==FALSE)
data2<-data2[,-67]
...
save(crdso,file='crdso.RData')
load('N:/data/Spera/crdso.RData')

a<-subset(crdso,select=c('lon','lat','bathy','dist'))
data<-merge(data,a,by=c('lon','lat'),all.x=TRUE,all.y=FALSE)
data2<-merge(data2,a,by=c('lon','lat'),all.x=TRUE,all.y=FALSE)


data$maxdepth<-ifelse(data$startdepth>data$enddepth,data$startdepth,data$enddepth)
