...
setwd('C:/Users/copepod/Documents/aalldocuments/literature/research/ACTIVE/spera/Figures')
setwd('C:/Users/sailfish/Documents/aalldocuments/literature/research/ACTIVE/spera/Figures')
pdf('div4xw_map.pdf',height=4,width=4)
map('world',col='gray',fill=TRUE,lwd=.1,border=NA,xlim=c(-68,-59),ylim=c(42,46))
plot(fourxw,col='royalblue',border=NA,add=TRUE)
axis(1,seq(-69,-58,1),las=1,cex.axis=.75,lwd=.1)
axis(2,seq(38,46,1),las=1,cex.axis=.75,lwd=.1)
box(lwd=.1)
dev.off()
...
cst<-crop(cst,extent(-70,-59,40,50))
...
points(dat$lon,dat$lat,pch=16,col='blue')
...
map('world',xlim=c(-70,-59),ylim=c(39,46))
map.axes()
...
map('world',xlim=c(-70,-55),ylim=c(39,46))
map.axes()
...
#save(data,file='phytoplankton_biomass_all_spera_v2.RData')
save(data,file='phytoplankton_biomass_all_spera_v2.RData')
load('phytoplankton_biomass_all_spera.RData')
crd<-unique(subset(data,select=c('lon','lat')))
plot(crd$lon,crd$lat,pch=16)
