...
data$q<-data$her.totwgt.rv/data$her.ssb
plot(data$year,data$q,pch=15)
...
rvl2<-rvl
#rvl2$lcat<-cut(rvl$flen,breaks=seq(5,45,5),labels=seq(7.5,42.5,5))
rvl2$lcat<-cut(rvl$flen,breaks=seq(5,44,3),labels=seq(6.5,42.5,3))
rvl2$id<-gsub(' ','',paste(rvl2$mission,'_',rvl2$setno))
rvl2$id1<-gsub(' ','',paste(rvl2$mission,'_',rvl2$setno,'_',rvl2$fshno))

rvl2$lcat<-as.numeric(as.character(rvl2$lcat))
a<-subset(rvl2,lcat<32)
b<-subset(rvl2,lcat>32)
...
d<-subset(rvll,id==sort(unique(rvll$id))[1])

#ADDS 0'S FOR EACH LENGTH CATEGORY
dt<-unique(subset(rvll,select=c('lcat')))
f<-function(d){
d2<-merge(dt,d,by=c('lcat'),all.x=TRUE,all.y=FALSE)
#d2<-rbind.fill(dt,d)
d2$id<-unique(d$id)
...
modw<-gam(wt ~ as.factor(year) + s(lon,lat,k=10) + s(time,bs='cc',k=5),data=d,family='nb')
modn<-gam(no ~ as.factor(year) + s(lon,lat,k=10) + s(time,bs='cc',k=5),data=d,family='nb')
...
ot<-ddply(rvll2,.(lcat),.fun=f,.progress='text')

ot$lcat<-as.numeric(as.character(ot$lcat))
ot$lpn<-log10(ot$pn+1)
ot$lpw<-log10(ot$pw+.01)

#plot(ot$lcat,ot$year,pch=16,cex=rescale(ot$lpn,newrange=c(.2,5)),col=alpha('darkred',.75),las=1)
dt$lcat<-as.numeric(as.character(dt$lcat))

pltfun<-function(ott,ttl){
names(ott)[1]<-'y'
return(ggplot()+
geom_tile(data=ott, aes(x=lcat, y=year,fill=y),col='gray80',size=.0001)+
...
scale_x_continuous(expand=c(0,0),breaks=sort(dt$lcat),labels=sort(dt$lcat),limits=NA)+
scale_y_continuous(expand=c(0,0),breaks=seq(1970,2015,5),labels=as.character(seq(1970,2015,5)),limits=NA)+
coord_equal()+
coord_cartesian(ylim=c(1969.5,2016.5),xlim=c(5,44))+
    xlab('')+
    ylab('')+
         labs(title = ttl,
       subtitle = "",
...
plnum<-pltfun(subset(ot,select=c('lpn','lcat','year')),'Log numbers')
plwgt<-pltfun(subset(ot,select=c('lpw','lcat','year')),'Log weight')
pnum<-pltfun(subset(ot,select=c('pn','lcat','year')),'Numbers')
pwgt<-pltfun(subset(ot,select=c('pw','lcat','year')),'Weight')
...
plot(rvw$lon,rvw$lat,pch=16,col=alpha('darkred',.2))
...
ff<-function(d){
shf<-subset(dayshift,strat==unique(d$strat))$day
print(shf)
names(d)[1]<-'y'
...
#TOTAL WEIGHT
out<-ddply(subset(rvw2, strat %in% dmm$strat,select=c('totwgt','strat','year','day','time')),.(strat),.fun=ff,.progress='text')
xyplot(pz~day|strat,data=out,col=ifelse(out$did==1,'red','blue'))
xyplot(pz~day|as.factor(lat),data=out,col=ifelse(out$did==1,'red','blue'))
xyplot(pz~day|as.factor(lon),data=out,col=ifelse(out$did==1,'red','blue'))
...
#SHIFT DAYS TO CENTER ON JULY AND MAKE CONTINUOUS
d<-dayshiftfun(d,shf)
#mod<-gam(y~s(year,k=4) + s(time,k=4,bs='cc') + s(day,k=5,bs='cc'),data=d,gamma=1.4)
mod<-gam(y~s(year) + s(time,k=4,bs='cc') + s(day2,k=4,bs='cc'),data=d,gamma=1.4,family='nb'(link='log'))
    pdat<-data.frame(year=max(d$year),
                     time=1200,
                     day2=seq(min(d$day2),max(d$day2),1))
    ...
    pdat$p<-(pdat$p-mean(pdat$p))/sd(pdat$p)
    pdat<-revdayshiftfun(pdat,shf)
    pdat<-subset(pdat,select=c('day3','p'))
names(pdat)<-c('day',gsub(' ','',paste('X',unique(as.character(d$strat)))))
    pdat<-pdat[order(pdat$day),]
return(data.frame(pdat))
}
#TOTAL WEIGHT
qq<-dlply(subset(rvw2, strat %in% dmm$strat,select=c('totwgt','strat','year','day','time')),.(strat),.fun=fsm)
qsm<-Reduce(function(x, y) merge(x, y, by=c('day'),all=TRUE), qq)#COMBINE
qsm<-qsm[,colSums(is.na(qsm)) != nrow(qsm)]#REMOVES COLUMNS THAT ARE ALL MISSING

q<-qsm
q2<- q %>% gather(strata, value, -day)
xyplot(value ~ day | strata,data=q2, pch=15, type=c('spline'),col='black')
xyplot(value ~ year | strata,data=q2, pch=15, type=c('p','spline'),col='black')

df<-qsm
k<-4
lbl<-"Total observed herring (weight)"

clfun<-function(df,k,lbl){
rownames(df)<-df$day
df<-df[,-1]
df<-df[,!(names(df) %in% c('Var.2'))]
...
par(mfrow=c(2,1),mar=c(4,4,1,1))
dum<-c('red3','forestgreen','darkblue','gold3','darkblue')
plot(silhouette(ff),col=dum[1:k],main='')#silhouette plot
...
a2<-a
if(k==3){a2$cx<- apply(aa[,c('X1','X2','X3')], 1, function(x) max(x) )
 } else {a2$cx<- apply(aa[,c('X1','X2','X3','X4')], 1, function(x) max(x) )
     }
a2$id<-gsub('X','',a2$strat)
am<-fortify(plg,region='stratum')
am<-subset(am,id%in% a2$id)
mydat<-merge(am,a2,by=c('id'))
mydat$cx<-rescale(mydat$cx,newrange=c(.4,1))
...
geom_polygon(aes(long,lat,group=group,fill=as.factor(clusters),alpha=cx),data=mydat,col='black',size=.0001) +
    coord_equal()+
    scale_fill_manual(values=c(dum[1],dum[2],dum[3]))+
#    scale_fill_manual(values=c(dum[1],dum[2],dum[3],dum[4]))+
    geom_polygon(aes(long,lat,group=group),fill="grey65", data=coast.mc)+
theme(panel.grid.major=element_blank(),panel.grid.minor=element_blank(),legend.position='bottomright',plot.background=element_blank(),axis.line = element_line(color = 'black'), legend.key.size =  unit(0.15, "in"),legend.text=element_text(size=6))+
scale_x_continuous(expand=c(0,0),breaks=seq(-68,-57,1),labels=as.character(seq(-68,-57,1)),limits=NA)+
scale_y_continuous(expand=c(0,0),breaks=seq(42,47,1),labels=as.character(seq(42,47,1)),limits=NA)+
...
setwd(figsdir)
pdf('herring_rvsurvey_cluster_phenology.pdf',height=10,width=8)
mp1<-clfun(qsm,3,"Total observed herring (weight)")
dev.off()
...
library(magick)
library(gganimate)
qt2<-subset(out,select=c('strat','day','p','pz'))
qt2$id<-qt2$strat
...
p<-ggplot(mydat,aes(x=long,y=lat,group = group, fill=pz,frame=day)) +
theme_opts +
  coord_equal() +
  geom_polygon(color = 'grey',size=.0001) +
#  geom_polygon(aes(long,lat,group=group),fill=NA,colour='black',data=a) +
...
a<-subset(rvw,month<=3)
a<-subset(rvw,month>=9)
a<-rvw2
mod<-gam(totwgt~as.factor(year) + s(lon,lat,k=50) + s(time,bs='cc',k=4),data=a,family='nb'(link='log'),gamma=1.4)
mod<-gam(totwgt~as.factor(year) + s(day,bs='cc',k=5) +s(lon,lat,k=50) + s(time,bs='cc',k=4),data=a,family='nb'(link='log'),gamma=1.4)
...
p<-predict(mod,newdata=pdat,type='response')
pdat$p<-p
plot(pdat$year,pdat$p,pch=15)
plot(pdat$year,log10(pdat$p+1),pch=15)

f<-function(d){
return(data.frame(lon=median(d$lon),
                  lat=median(d$lat),
                  twgt=mean(d$totwgt)))
}
ot<-ddply(a,.(strat),.fun=f)

qt2<-subset(ot,select=c('strat','twgt'))
qt2$id<-qt2$strat
...
am<-subset(am,id%in% qt2$id)
mydat<-merge(am,qt2,by=c('id'))
mydat$ltwgt<-log10(mydat$twgt+1)

p<-ggplot(mydat,aes(x=long,y=lat,group = group, fill=ltwgt)) +
theme_opts +
  coord_equal() +
  geom_polygon(color = 'grey',size=.0001) +
  labs(title = "Herring presence between 1970 and 2015",
